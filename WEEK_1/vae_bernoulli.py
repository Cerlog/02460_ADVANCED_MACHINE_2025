...
class MoGPrior(nn.Module):
    ...
    def forward(self):
        ...
        return td.MixtureSameFamily(
            td.Categorical(probs=self.weights),
            td.Independent(td.Normal(loc=self.means, scale=self.stds), 1)
        )
...
class VAE(nn.Module):
    ...
    def elbo(self, x):
        ...
        """
        q = self.encoder(x)
        #print(f"Dimension of Encoder output: {q.mean.shape}")
        z = q.rsample()
        #########################################
        # Printing out the dimension of z and its mean and std
        #print(f"Dimension of z: {z.shape}")
        #print(f"The mean and std of z: {z.mean()}, {z.std()}")
        #########################################
        # Finding the decoder shape 
        #decoders = self.decoder(z).log_prob(x)
        #print("Decorer: ", decoders.shape)#
        #########################################
        # Finding the KL divergence term 
...
def visualize_latent_space(model, data_loader, device, M):
    """
    Sample from the approximate posterior q(z|x) and visualize it in 2D.

    Parameters:
    - model: Trained VAE model (should be loaded outside this function)
    ...
    latent_samples = np.concatenate(latent_samples, axis=0)  # Shape (N, M)
    ...
    if M > 2:
        ...
        latent_samples = pca.fit_transform(latent_samples)  # Shape (N, 2)
...
if __name__ == "__main__":
    ...
    parser.add_argument('mode', type=str, default='train', choices=['train', 'sample', 'evaluate'], help='what to do when running the script (default: %(default)s)')
    ...
    if args.mode == 'train':
        ...
        print(f"ELBO on training set: {evaluate_elbo(model, mnist_train_loader, args.device)}")
        print(f"ELBO on test set: {evaluate_elbo(model, mnist_test_loader, args.device)}")
        
        visualize_latent_space(model, mnist_test_loader, args.device, M)
